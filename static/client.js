...
body.addEventListener('submit', function(ev){
  ...
  invite(coc && coc.checked ? 1 : 0, email.value, first_name.value, last_name.value, document.querySelector('[name="cf-turnstile-response"]').value, function(err){
    if (err) {
...
function invite(coc, email, first_name, last_name, turnstile_res, fn){
  request
  .post('/invite/')
  .type('form')
  .send({
    ...
    fname: first_name,
    lname: last_name,
    "cf-turnstile-response": turnstile_res
  })
  .end(function(res){
      console.log(res);
    if (res.error) {
